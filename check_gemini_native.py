...
def main():
    ...
    print("\n2. Testing the native Gemini API with a short test prompt...")
    result = call("/v1beta/models/" + model + ":generateContent", key, {
        "systemInstruction": {"parts": [{"text": "You are a connection test assistant."}]},
        "contents": [{"role": "user", "parts": [{"text": "Reply with only OK."}]}]
    }, native=True)
    try:
        from utils.gemini_native import response_text
        response_text(result)
        print("SUCCESS: Native Gemini generation works.")
        print("Restart QUTwin, clear session AI settings, and retry your saved upload.")
        return 0
